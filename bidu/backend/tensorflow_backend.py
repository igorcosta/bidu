...
# This is the default internal TF session used by bidu.
# It can be set manually via `set_session(sess)`.
_SESSION = None
# This dictionary holds a mapping {graph: learning_phase}.
# A learning phase is a bool tensor used to run bidu models in
...
def clear_session():
    ...
    reset_uids()
    _SESSION = None
    phase = tf.placeholder(dtype='bool', name='bidu_learning_phase')
...
def learning_phase():
    ...
    to be passed as input to any bidu function
    that uses a different behavior at train time and test time.
    '''
    graph = tf.get_default_graph()
    if graph not in _GRAPH_LEARNING_PHASES:
        phase = tf.placeholder(dtype='bool',
                               name='bidu_learning_phase')
...
def get_session():
    ...
    Else, we will return the global bidu session.

    If no global bidu session exists at this point:
...
def _initialize_variables():
    ...
    for v in variables:
        if not hasattr(v, '_bidu_initialized') or not v._bidu_initialized:
            uninitialized_variables.append(v)
            v._bidu_initialized = True
...
def categorical_crossentropy(output, target, from_logits=False):
    ...
    # expects logits, bidu expects probabilities.
    if not from_logits:
...
# and therefore requires a wrapper for bidu. The functions below convert
