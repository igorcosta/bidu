...
class Node(object):
    ...
    def create_node(cls, outbound_layer,
        ...
        for i in range(len(output_tensors)):
            output_tensors[i]._bidu_shape = output_shapes[i]
            ...
            output_tensors[i]._bidu_history = (outbound_layer, len(outbound_layer.inbound_nodes), i)
...
class Layer(object):
    ...
    def assert_input_compatibility(self, input):
        ...
        for input_index, (x, spec) in enumerate(zip(inputs, self.input_spec)):
            ...
            if spec.shape is not None:
                if hasattr(x, '_bidu_shape'):
                    x_shape = x._bidu_shape
    ...
    def __call__(self, x, mask=None):
        ...
        internal bidu references.

        If a bidu tensor is passed:
            - We call self.add_inbound_node().
            - If necessary, we `build` the layer to match
                the _bidu_shape of the input(s).
            - We update the _bidu_shape of every input tensor with
                its new shape (obtained via self.get_output_shape_for).
                This is done as part of add_inbound_node().
            - We update the _bidu_history of the output tensor(s)
        ...
        if not self.built:
            ...
            for x_elem in to_list(x):
                if hasattr(x_elem, '_bidu_shape'):
                    input_shapes.append(x_elem._bidu_shape)
                elif hasattr(K, 'int_shape'):
        ...
        for input_tensor in input_tensors:
            if hasattr(input_tensor, '_bidu_history') and input_tensor._bidu_history:
                # This is a bidu tensor.
                previous_layer, node_index, tensor_index = input_tensor._bidu_history
        ...
        if inbound_layers:
            ...
            # This case appears if the input was not a bidu tensor.
    ...
    def build(self, input_shape):
        ...
        # Arguments
            input_shape: bidu tensor (future input to layer)
                or list/tuple of bidu tensors to reference
...
class InputLayer(Layer):
    ...
    def __init__(self, input_shape=None, batch_input_shape=None,
        ...
        if input_tensor is None:
            ...
            input_tensor._bidu_shape = batch_input_shape
        # Create an input node to add to self.outbound_node
        # and set output_tensors' _bidu_history.
        input_tensor._uses_learning_phase = False
        input_tensor._bidu_history = (self, 0, 0)
        Node(self,
             inbound_layers=[],
             node_indices=[],
             tensor_indices=[],
...
def Input(shape=None, batch_shape=None,
          ...
          tensor=None):
    '''`Input()` is used to instantiate a bidu tensor.
    A bidu tensor is a tensor object from the underlying backend
    (Theano or TensorFlow), which we augment with certain
    attributes that allow us to build a bidu model
    just by knowing the inputs and outputs of the model.

    For instance, if a, b and c and bidu tensors,
    it becomes possible to do:
    `model = Model(input=[a, b], output=c)`

    The added bidu attributes are:
        ._bidu_shape: Integer shape tuple propagated
            via bidu-side shape inference.
        ._bidu_history: Last layer applied to the tensor.
    ...
    # Example usage
        ...
        ```python
        # this is a logistic regression in bidu
        a = Input(shape=(32,))
    ...
    input_layer = InputLayer(batch_input_shape=batch_shape,
                             ...
                             sparse=sparse,
                             input_tensor=tensor)
    # Return tensor including _bidu_shape and _bidu_history.
...
class Merge(Layer):
    ...
    # Arguments
        layers: Can be a list of bidu tensors or
    ...
    def __call__(self, inputs, mask=None):
        ...
        all_bidu_tensors = True
        for x in inputs:
            if not hasattr(x, '_bidu_history'):
                all_bidu_tensors = False
                break
        ...
        if all_bidu_tensors:
            ...
            tensor_indices = []
            for x in inputs:
                layer, node_index, tensor_index = x._bidu_history
                layers.append(layer)
                node_indices.append(node_index)
                tensor_indices.append(tensor_index)
...
def merge(inputs, mode='sum', concat_axis=-1,
    ...
    '''Functional merge, to apply to bidu tensors (NOT layers).
    Returns a bidu tensor.
    ...
    if all_bidu_tensors:
        ...
        tensor_indices = []
        for x in inputs:
            input_layer, node_index, tensor_index = x._bidu_history
...
class Container(Layer):
    ...
    def __init__(self, input, output, name=None):
        ...
        for x in self.inputs:
            # Check that x is a bidu tensor.
            if not hasattr(x, '_bidu_history'):
                cls_name = self.__class__.__name__
                raise Exception('Input tensors to a ' + cls_name + ' ' +
                                'must be bidu tensors. Found: ' + str(x) +
                                ' (missing bidu metadata).')
        ...
        self.internal_input_shapes = [x._bidu_shape for x in self.inputs]
        self.internal_output_shapes = [x._bidu_shape for x in self.outputs]
        ...
        def build_map_of_graph(tensor, seen_nodes=set(), depth=0,
            ...
            # Arguments
                ...
                layer: Layer from which `tensor` comes from. If not provided,
                    will be obtained from `tensor._bidu_history`.
                node_index: Node index from which `tensor` comes from.
                tensor_index: Tensor_index from which `tensor` comes from.
            '''
            if not layer or node_index is None or tensor_index is None:
                layer, node_index, tensor_index = tensor._bidu_history
            node = layer.inbound_nodes[node_index]
        ...
        Node(outbound_layer=self,
             ...
             input_shapes=[x._bidu_shape for x in self.inputs],
             output_shapes=[x._bidu_shape for x in self.outputs])
        self.built = True
        self.supports_masking = False
    ...
    def call(self, input, mask=None):
        ...
        It is callable on non-bidu tensors.
    ...
    def run_internal_graph(self, inputs, masks=None):
        ...
        # Note:
            ...
            - Can be run on non-bidu tensors.
        ...
        for depth in depth_keys:
            ...
            for node in nodes:
                ...
                if len(computed_data) == len(reference_input_tensors):
                    ...
                    # Update _bidu_shape.
                    if all([hasattr(x, '_bidu_shape') for x in computed_tensors]):
                        if len(computed_tensors) == 1:
                            shapes = to_list(layer.get_output_shape_for(computed_tensors[0]._bidu_shape))
                            uses_learning_phase = computed_tensors[0]._uses_learning_phase or layer.uses_learning_phase
                        else:
                            shapes = to_list(layer.get_output_shape_for([x._bidu_shape for x in computed_tensors]))
        ...
        for x in self.outputs:
            ...
            if hasattr(tensor, '_bidu_shape') and output_shapes is not None:
                shape = tensor._bidu_shape
        ...
        if output_shapes is not None:
            input_shapes = [x._bidu_shape for x in inputs]
            cache_key = ','.join([str(x) for x in input_shapes])
            if len(output_shapes) == 1:
    ...
    def from_config(cls, config, custom_objects={}):
        ...
        '''
        from bidu.utils.layer_utils import layer_from_config
    ...
    def save(self, filepath, overwrite=True):
        ...
        Saved models can be reinstantiated via `bidu.models.load_model`.
        ...
        ```python
        from bidu.models import load_model
    ...
    def _updated_config(self):
        ...
        from bidu import __version__ as bidu_version

        config = self.get_config()
        model_config = {
            'class_name': self.__class__.__name__,
            'config': config,
            'bidu_version': bidu_version
        }
        return model_config
    ...
    def to_json(self, **kwargs):
        ...
        `bidu.models.model_from_json(json_string, custom_objects={})`.
        '''
        import json
    ...
    def to_yaml(self, **kwargs):
        ...
        `bidu.models.model_from_yaml(yaml_string, custom_objects={})`.
    ...
    def summary(self, line_length=100, positions=[.33, .55, .67, 1.]):
        from bidu.utils.layer_utils import print_summary
...
def get_source_inputs(tensor, layer=None, node_index=None):
    ...
    # Arguments
        ...
        layer: Origin layer of the tensor. Will be
            determined via tensor._bidu_history if not provided.
        node_index: Origin node index of the tensor.
    '''
    if not hasattr(tensor, '_bidu_history'):
        raise Exception('Tensor must be a bidu tensor. Found: ' + str(tensor))

    if layer is None or node_index:
        layer, node_index, _ = tensor._bidu_history
