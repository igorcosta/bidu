# Getting started with the bidu functional API

The bidu functional API is the way to go for defining complex models, such as multi-output models, directed acyclic graphs, or models with shared layers.
...
- Such a model can be trained just like bidu `Sequential` models.
...
```python
from bidu.layers import TimeDistributed
...
<img src="https://s3.amazonaws.com/bidu.io/img/multi-input-multi-output-graph.png" alt="multi-input-multi-output-graph" style="width: 400px;"/>
...
```python
from bidu.layers import Input, Embedding, LSTM, Dense, merge
from bidu.models import Model
...
```python
from bidu.layers import Input, LSTM, Dense, merge
from bidu.models import Model
...
In previous versions of bidu, you could obtain the output tensor of a layer instance via `layer.get_output()`, or its output shape via `layer.output_shape`. You still can (except `get_output()` has been replaced by the property `output`). But what if a layer is connected to multiple inputs?
...
```python
from bidu.layers import merge, Convolution2D, MaxPooling2D, Input
...
```python
from bidu.layers import merge, Convolution2D, Input
...
```python
from bidu.layers import merge, Convolution2D, MaxPooling2D, Input, Dense, Flatten
from bidu.models import Model
...
```python
from bidu.layers import Convolution2D, MaxPooling2D, Flatten
from bidu.layers import Input, LSTM, Embedding, Dense, merge
from bidu.models import Model, Sequential
