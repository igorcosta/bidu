...
```python
bidu.preprocessing.sequence.pad_sequences(sequences, maxlen=None, dtype='int32')
```
...
```python
bidu.preprocessing.sequence.skipgrams(sequence, vocabulary_size,
...
```python
bidu.preprocessing.sequence.make_sampling_table(size, sampling_factor=1e-5)
```
