...
```python
bidu.preprocessing.text.text_to_word_sequence(text, 
...
```python
bidu.preprocessing.text.one_hot(text, n,
    filters=base_filter(), lower=True, split=" ")
```
...
```python
bidu.preprocessing.text.Tokenizer(nb_words=None, filters=base_filter(), 
    lower=True, split=" ")
```
