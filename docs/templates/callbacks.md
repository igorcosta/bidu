...
You can create a custom callback by extending the base class `bidu.callbacks.Callback`. A callback has access to its associated model through the class property `self.model`.

Here's a simple example saving a list of losses over each batch during training:
```python
class LossHistory(bidu.callbacks.Callback):
...
```python
from bidu.callbacks import ModelCheckpoint
