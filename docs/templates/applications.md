...
bidu Applications are deep learning models that are made available alongside pre-trained weights.
These models can be used for prediction, feature extraction, and fine-tuning.

Weights are downloaded automatically when instantiating a model. They are stored at `~/.bidu/models/`.
...
All of these architectures (except Xception) are compatible with both TensorFlow and Theano, and upon instantiation the models will be built according to the image dimension ordering set in your bidu configuration file at `~/.bidu/bidu.json`. For instance, if you have set `image_dim_ordering=tf`, then any model loaded from this repository will get built according to the TensorFlow dimension ordering convention, "Width-Height-Depth".
...
from bidu.preprocessing import image
from bidu.applications.resnet50 import preprocess_input, decode_predictions
import numpy as np
...
from bidu.preprocessing import image
from bidu.applications.vgg16 import preprocess_input
import numpy as np
...
from bidu.preprocessing import image
from bidu.applications.vgg19 import preprocess_input
...
from bidu.preprocessing import image
from bidu.models import Model
from bidu.layers import Dense, GlobalAveragePooling2D
...
```python
from bidu.applications.inception_v3 import InceptionV3
from bidu.layers import Input

# this could also be the output a different bidu model or layer
...
```python
bidu.applications.xception.Xception(include_top=True, weights='imagenet', input_tensor=None)
```
...
- input_tensor: optional bidu tensor (i.e. output of `layers.Input()`) to use as image input for the model.

### Returns

A bidu model instance.
...
```python
bidu.applications.vgg16.VGG16(include_top=True, weights='imagenet', input_tensor=None)
```
...
```python
bidu.applications.vgg19.VGG19(include_top=True, weights='imagenet', input_tensor=None)
```
...
```python
bidu.applications.resnet50.ResNet50(include_top=True, weights='imagenet', input_tensor=None)
```
...
```python
bidu.applications.inception_v3.InceptionV3(include_top=True, weights='imagenet', input_tensor=None)
```
...
```python
bidu.applications.music_tagger_crnn.MusicTaggerCRNN(weights='msd', input_tensor=None, include_top=True)
```

A convolutional-recurrent model taking as input a vectorized representation of the MelSpectrogram of a music track and capable of outputting the musical genre of the track. You can use `bidu.applications.music_tagger_crnn.preprocess_input` to convert a sound file to a vectorized spectrogram. This requires to have installed the [Librosa](http://librosa.github.io/librosa/) library. See [the usage example](#music-tagging-and-feature-extraction-with-musictaggercrnn).
...
These weights are ported from the ones [released by Keunwoo Choi](https://github.com/keunwoochoi/music-auto_tagging-bidu) under the [MIT license](https://github.com/keunwoochoi/music-auto_tagging-bidu/blob/master/LICENSE.md).

### Examples: music tagging and audio feature extraction

```python
from bidu.applications.music_tagger_crnn import MusicTaggerCRNN
from bidu.applications.music_tagger_crnn import preprocess_input, decode_predictions
import numpy as np
