...
from bidu.layers import Dense, Dropout, InputLayer
from bidu.engine import merge, Input, get_source_inputs
from bidu.models import Model, Sequential
from bidu import backend as K
from bidu.models import model_from_json, model_from_yaml
...
def test_node_construction():
    ...
    assert a._bidu_shape == (None, 32)
    a_layer, a_node_index, a_tensor_index = a._bidu_history
    b_layer, b_node_index, b_tensor_index = b._bidu_history
...
def test_multi_input_layer():
    ...
    assert merged._bidu_shape == (None, 16 * 2)
    merge_layer, merge_node_index, merge_tensor_index = merged._bidu_history
...
def test_recursion():
    ...
    assert g._bidu_shape == c._bidu_shape
    assert h._bidu_shape == d._bidu_shape
    ...
    assert n._bidu_shape == (None, 5)
    assert q._bidu_shape == (None, 64)
    s = merge([n, q], mode='concat', name='merge_nq')
    assert s._bidu_shape == (None, 64 + 5)
    ...
    if K._BACKEND == 'tensorflow':
        ...
        assert not hasattr(m_tf, '_bidu_shape')
        assert not hasattr(n_tf, '_bidu_shape')
...
#     # MNIST
#     from bidu.layers import Input, Dense, LSTM
#     from bidu.models import Model
#     from bidu.utils import np_utils
...
def test_sequential_regression():
    from bidu.models import Sequential, Model
    from bidu.layers import Merge, Embedding, BatchNormalization, LSTM, InputLayer, Input
