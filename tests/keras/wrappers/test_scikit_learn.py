...
from bidu.utils.test_utils import get_test_data
from bidu.utils import np_utils
from bidu import backend as K

from bidu.models import Sequential
from bidu.layers.core import Dense, Activation
from bidu.wrappers.scikit_learn import biduClassifier, biduRegressor
...
def test_clasify_inherit_class_build_fn():
    class InheritClassBuildFnClf(biduClassifier):
        def __call__(self, hidden_dims):
            return build_fn_clf(hidden_dims)
...
def test_regression_inherit_class_build_fn():
    class InheritClassBuildFnReg(biduRegressor):
        def __call__(self, hidden_dims):
            return build_fn_reg(hidden_dims)
